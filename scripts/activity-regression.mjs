...
localStorage.setItem(ACTIVITY_KEY, JSON.stringify([base, { ...base, id: '', exerciseLogIds: [] }]));
assert.deepEqual(readActivities(), { activities: [base], error: false }, 'one malformed record does not hide valid activity history');
assert.equal(saveActivity({ ...base, id: 'after-malformed', exerciseLogIds: [] }), true, 'one malformed record does not block future saves');
