...
export default function BodyTrainingEntry({ onActivitiesChange }: { onActivitiesChange: () => void }) {
  ...
  return <section className="card space-y-4 p-4" aria-labelledby="body-entry-title">
    ...
    <p className="text-sm text-slate-600">{t('bodyEntry.intro')}</p>
    <div className="grid grid-cols-3 gap-2" role="group" aria-label={t('bodyEntry.modeLabel')}>
    ...
    {kind === 'cycling' ? <>
      ...
      <p className="rounded-md bg-calm-50 p-3 text-sm font-semibold text-calm-900">{t(`bodyEntry.${kind}Hint`)}</p>
