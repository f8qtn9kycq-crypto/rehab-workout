...
import { isValidYouTubeEmbedUrl, toYouTubeEmbedUrl } from '../services/videoUtils';
  ...
  if (!isValid) {
    return (
      <div className="rounded-lg border border-dashed border-slate-300 bg-slate-50 p-4 text-sm text-slate-600">
        {t('video.fallback')}
        {fallbackUrl ? (
          <a className="focus-ring mt-3 inline-flex min-h-11 items-center justify-center rounded-md bg-white px-3 font-semibold text-calm-700" href={fallbackUrl} target="_blank" rel="noreferrer">
            {t('video.search')}
          </a>
        ) : null}
      </div>
