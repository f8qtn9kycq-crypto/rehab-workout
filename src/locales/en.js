export default {
  muscleEntry: {
    map: 'Resistance training muscle map', choose: 'Select muscles: chest, shoulders and lower body on front; back and lower body on rear',
    chest: 'Chest', shoulders: 'Shoulders', back: 'Back', legs: 'Lower body',
    chestHint: 'Movement type: bench press', shouldersHint: 'Movement type: shoulder press',
    backHint: 'Movement type: pull / row', legsHint: 'Movement type: hips, glutes, legs, knees and ankles',
    catalogNote: 'Related exercises are listed here; some resistance movements are searchable only in All exercises.',
    empty: 'No exercise is available for this muscle group yet.',
  },
  bodyEntry: {
    title: 'What would you like to do today?', front: 'Front', back: 'Back',
    intro: 'Choose your goal first, then find the area on the body image.', modeLabel: "Choose today's goal",
    rehab: 'Rehab', resistance: 'Strength', cycling: 'Cycling',
    choose: 'Select an area on the body image', selected: 'Selected: {area}',
    rehabHint: 'Select the area that feels sore to see relevant rehab exercises. Safety checks still apply before training.',
    resistanceHint: 'Select the muscle group you want to train to see available resistance exercises.',
    recordHint: 'After training, log pain, weight and repetitions for each set.',
    empty: 'No resistance exercise is available for this area yet. Try Rehab.',
  },
  activities: {

  "exerciseRecords": "{count} exercise records",
  "feedback": "Worse symptoms / red flags: {worse}; next-day responses missing: {missing}; awaiting tomorrow: {pending}."
,

  "plan": "Seven-day plan",
  "planHint": "Adjust days as needed. Lower/push/pull are flexible focuses, not exercise prescriptions. Scheduling never marks an activity complete.",
  "rest": "Rest",
  "editPlan": "Edit schedule",
  "days": {
    "0": "Monday",
    "1": "Tuesday",
    "2": "Wednesday",
    "3": "Thursday",
    "4": "Friday",
    "5": "Saturday",
    "6": "Sunday"
  },
  "planNames": {
    "lower": "Resistance (lower focus)",
    "push": "Resistance (push focus)",
    "pull": "Resistance (pull focus)",
    "cycling": "Cycling 15 minutes"
  }
,
  "title": "Today and this week",
  "summary": "Resistance {resistance}/3 · Cycling {cycling}/4 rides, {minutes} minutes",
  "legacy": "Existing logs are preserved. Ungrouped exercise logs do not count as resistance sessions.",
  "resistance": "Record resistance",
  "cycling": "Record cycling",
  "recordOnly": "Record an activity that already happened, including incomplete attempts.",
  "startSafely": "Start guided training: safety check first",
  "date": "Activity date",
  "focus": "Primary focus (mixed exercises allowed)",
  "unifiedSessionHint": "One session can combine prep, main strength, accessories and conditioning. Link the exercise logs from this visit.",
  "phase": "Session part",
  "quality": "Performance quality",
  "linkLogs": "Link same-day exercise logs (optional, multiple allowed)",
  "completed": "Completed",
  "incomplete": "Incomplete",
  "minutes": "Actual minutes",
  "response": "Shoulder, hip or other symptoms compared with before",
  "choose": "Choose a response",
  "warning": "Stop if pain increases, numbness, weakness or instability occurs. Consider checking with a physician or physical therapist. Stopped exercises and pain records are included conservatively.",
  "save": "Save activity",
  "cancel": "Cancel",
  "history": "Activities and next-day feedback",
  "minuteUnit": "minutes",
  "logUnit": "exercise logs",
  "nextDay": "Next-day response (optional, enter the following day)",
  "unknown": "Not reported",
  "share": "Copyable weekly summary",
  "localOnly": "Select summary text to copy. Data stays in this browser and does not sync to ChatGPT.",
  "saved": "Saved",
  "error": "Unable to read or save activities. Existing data was not overwritten. Check browser storage and permissions.",
  "focuses": {
    "lower": "Lower body",
    "push": "Push",
    "pull": "Pull",
    "mixed": "Mixed"
  },
  "phases": { "prep": "Prep / rehab", "main": "Main strength", "accessory": "Accessory", "conditioning": "Conditioning", "cooldown": "Cool-down" },
  "qualities": { "controlled": "Controlled", "no_reps": "No-reps noted", "pain_limited": "Pain-limited" },
  "responses": {
    "same": "Unchanged",
    "better": "Better",
    "worse": "Worse / poor recovery",
    "red_flag": "Pain at least 6 or red flags"
  },
  "recommendations": {
    "waiting": "Targets met. Today’s activities are awaiting next-day observation. Maintain and record the response tomorrow.",
    "consistency": "Build consistency toward 3 resistance sessions and 4 rides weekly. Do not make up missed activities with extra volume.",
    "stop": "Stop training and consider checking with a physician or physical therapist. Completion does not replace a safety check.",
    "reduce": "Symptoms worsened or training stopped early. Maintain or use an easier option following your physical therapist’s guidance; do not add volume.",
    "missing": "Targets met, but next-day responses are missing. Maintain; do not assume good recovery.",
    "baseline": "Responses are stable but prior-week minutes are missing. Maintain and build a baseline.",
    "volume": "Minutes increased for at least one activity type. Maintain without adding another progression.",
    "small": "Consistency and responses are stable; minutes have not increased. Minutes do not measure load or sets. Only if those have not increased and your physical therapist’s guidance allows it, consider one extra rep per set in one familiar exercise; keep everything else unchanged."
  }
},
  entryDemo: {
    label: 'Demo scenarios',
    first: 'First visit',
    returning: 'Returning',
    notice: 'Demo only. Switching views does not change records. Returning uses your current safety status.',
  },
  app: {
    brand: 'Senior Rehab Training',
    languageLabel: 'Language',
    languageGroup: 'Language switcher',
    switchToChinese: 'Switch to Traditional Chinese',
    switchToEnglish: 'Switch to English',
    chinese: 'Chinese',
    english: 'English',
  },
  nav: {
    home: 'Home',
    safety: 'Safety',
    assessment: 'Assessment',
    exercises: 'Exercise Library',
    logs: 'Logs',
    education: 'Education',
    more: 'More',
    routine: 'Weekly routine',
    primary: 'Primary navigation',
    mobilePrimary: 'Mobile navigation',
    openMore: 'Open more navigation options',
    closeMore: 'Close more navigation options',
  },
  actions: {
    start: 'Start',
    continue: 'Continue last session',
    backToExercises: 'Back to Exercise Library',
    moreFilters: 'More filters',
    hideFilters: 'Hide filters',
    clearFilters: 'Clear filters',
    close: 'Close',
  },
  home: {
    safetyFirst: 'Safety first',
    title: 'Rehab and Strength Training',
    subtitle: 'Choose shoulder, hip, shoulder-neck, knee, or ankle routines after a simple safety check.',
    startSafely: 'Start safely',
    pickBodyArea: 'Pick body area',
    continueLastSession: 'Continue last training',
    weeklyProgress: 'Weekly progress',
    savedLogs: '{count} training logs saved on this device.',
    todayFocus: 'Today only needs one safe, controlled training session.',
    summaryLabel: 'Home summary',
    functionCheckTitle: 'Function check-in',
    outcomeSummary: '{count} function check-ins saved on this device.',
    routineTitle: 'Weekly routine',
    routineSummary: 'Use a short existing routine when you want a guided starting point.',
    nextAction: {
      label: 'Next step today',
      guardNote: 'Start actions still use the existing safety check and session flow.',
      safety: {
        title: 'Check how your body feels today',
        body: 'Before starting, check for any discomfort that needs attention today.',
        cta: 'Start safety check',
      },
      assessment: {
        title: 'How does movement feel today?',
        body: 'Check pain and movement confidence, then choose an exercise.',
        cta: 'Check today’s condition',
      },
      outcome: {
        title: 'Add a quick function check-in',
        body: 'You have recent training logs. A short function check-in helps compare how daily movement feels.',
        cta: 'Add check-in',
      },
      continue: {
        title: 'Continue from your latest training',
        body: 'Repeat the last exercise only if today still feels controlled and safe.',
        cta: 'Continue training',
      },
      start: {
        title: 'Start with today’s suitable exercises',
        body: 'Choose a body area and begin with slow, controlled, low-impact movement.',
        cta: 'Choose exercises',
      },
    },
  },
  weeklyRoutine: {
    title: 'Weekly training plan',
    subtitle: 'Choose a short plan built from existing gentle exercises. Starting an exercise still goes through the safety check.',
    recovery5: '5-min recovery',
    bodyArea10: '10-min body-area plan',
    starter3Day: '3-day starter',
    recovery5Helper: 'Use relaxation and gentle mobility when pain or fatigue is higher than usual.',
    bodyArea10Helper: "Below are short sets for each body area. Pick today's section and start gently.",
    starter3DayHelper: 'Three light days per week, designed for building a steady beginner habit.',
    monday: 'Monday',
    wednesday: 'Wednesday',
    friday: 'Friday',
    today: 'Today',
    shoulderDay: 'Shoulder',
    hipDay: 'Hip',
    kneeDay: 'Knee',
    ankleDay: 'Ankle',
    neckDay: 'Shoulder-neck',
    startExercise: 'Start this exercise',
    sessionGuardHint: 'Reminder: starting an exercise still uses the existing safety check and session flow.',
  },
  onboarding: {
    badge: 'Safety first',
    title: 'Start with one safe path today',
    subtitle: 'First confirm safety, then choose a body area and learn when to check pain before and after training.',
    stepsLabel: 'First-run path',
    steps: ['Safety check', 'Choose body area', 'Pain before and after', 'Start suitable exercise'],
    start: 'Start safety check',
  },
  bodyAreas: {
    shoulder: { label: 'Shoulder Rehab', hint: 'Mobility, rotator cuff, scapular control' },
    hip: { label: 'Hip Rehab', hint: 'Glutes, gait, sit-to-stand stability' },
    shoulder_neck: { label: 'Shoulder-Neck Relief', hint: 'Soreness, rounded shoulders, forward head' },
    knee: { label: 'Knee Relief', hint: 'Knee comfort and leg strength' },
    ankle: { label: 'Ankle Stability', hint: 'Stability, balance, mobility' },
  },
  exercises: {
    title: 'Exercise Library',
    subtitle: 'Start with recommended exercises or choose a body area. Written steps and safety notes stay visible.',
    startingTitle: 'Start with these exercises today',
    startingReason: 'Based on your selected area ({bodyArea}), current pain, and movement confidence, these options are easier to control.',
    filterTitle: 'Find exercises',
    filterHint: 'Choose today’s mode and body area. Level and equipment stay visible as reference labels.',
    mode: 'Mode',
    recommendedMode: "Today's suitable exercises",
    recommendedHelper: 'Filtered by your current status',
    allMode: 'All exercises',
    bodyArea: 'Body area',
    type: 'Type',
    level: 'Level',
    equipment: 'Equipment',
    supportEquipment: 'Support equipment',
    duration: 'Duration',
    painSensitivity: 'Pain sensitivity',
    anyDuration: 'Any duration',
    shortDuration: 'Short',
    mediumDuration: 'Medium',
    painSensitiveMode: 'Extra gentle if pain is above 3/10',
    allTypes: 'All types',
    allLevels: 'All levels',
    noEquipmentQuick: 'Bodyweight / No equipment',
    clearEquipment: 'Clear equipment',
    chooseBodyArea: 'Choose a body area to begin.',
    painStopEmpty: 'Your current pain score is high. Stop training for now and rest. If pain persists or reaches 6/10 or higher, consider consulting a physician or physical therapist.',
    activeFilters: 'Active filters',
    recommendedSummary: 'Suitable today',
    allSummary: 'All',
    advancedFilters: 'Advanced filters',
    bodyweightOnly: 'Bodyweight only',
    countBadge: '{count}',
    unavailableFilter: '{label}: no matching exercises yet',
    empty: 'No exercises match these filters. Try adjusting the filters.',
    emptyTitle: 'No exact match yet',
    emptyMessage: 'No exact match yet. Try "Bodyweight / chair or wall support," choose an easier level, or return to "Today’s suitable exercises."',
    emptyClearAction: 'Clear filters',
    emptyBodyweightAction: 'Try bodyweight exercises',
    emptyRecommendedAction: "Back to Today's suitable exercises",
    viewDetail: 'View exercise',
    setsUnit: 'sets',
    repsUnit: 'reps',
    restUnit: 'sec rest',
  },
  assessment: {
    title: 'Assessment',
    subtitle: 'This is not a diagnosis. It only helps choose a safer training level for today.',
    bodyAreaLabel: 'Choose a body area',
    painLabel: 'Current pain level',
    confidenceLabel: 'Movement confidence today',
    functionalLabel: 'How easy does daily movement feel today?',
    painLevels: { 0: 'No pain', 1: 'Very mild', 2: 'Mild', 3: 'Mild', 4: 'Moderate', 5: 'Moderate', 6: 'Moderate', 7: 'Strong', 8: 'Strong', 9: 'Very strong', 10: 'Worst pain' },
    confidenceLevels: { 0: 'No confidence', 1: 'Very low', 2: 'Low', 3: 'Limited', 4: 'Limited', 5: 'Manageable', 6: 'Steady', 7: 'Steady', 8: 'Strong', 9: 'Very strong', 10: 'Very confident' },
    equipmentLabel: 'Available equipment',
    timeLabel: 'Available time today',
    minutes: '{count} min',
    recommendation: 'Based on your pain and movement confidence, start with {mode} today using low-impact, slow, controlled movement.',
    recoveryMode: 'Recovery mode',
    beginnerPlan: 'Beginner plan',
    standardPlan: 'Standard plan',
    saveAndChoose: 'Save assessment and see exercises',
    saveNotice: 'Assessment data is stored in this browser on this device. You can clear it in Logs. Take care on shared devices.',
  },
  safety: {
    title: 'Pre-training safety check',
    subtitle: 'This content is for education and self-care support only. It does not diagnose, treat, or claim to cure.',
    whyTitle: 'Why check first?',
    whyBody: 'First confirm there is nothing that needs attention before exercise. Then we can show options that are easier to control based on pain and movement comfort.',
    blocked: 'You selected a condition that needs attention first. For safety, skip training today and consider consulting a physician or physical therapist.',
    ready: 'If none of these apply, begin with low intensity, slow speed, and controlled movement.',
    blockedAction: 'View safety guidance',
    readyAction: 'Confirm and continue',
    selectRequired: 'Choose whether any of the above apply today.',
    noneOfAbove: 'None of the above',
    noneOfAboveDescription: 'None of these priority safety concerns apply to me today.',
    redFlagGroup: 'Safety symptoms checklist',
    redFlagInstruction: 'Select any symptoms that apply today, or choose None of the above.',
    redFlags: {
      'acute-severe-pain': {
        label: 'Sudden severe pain',
        description: 'Pain is sudden, severe, or clearly worsens with small movements',
      },
      'neuro-symptoms': {
        label: 'Numbness, tingling, or weakness',
        description: 'Numbness, tingling, clear weakness, or unusual sensation in an arm or leg',
      },
      'systemic-symptoms': {
        label: 'Fever, dizziness, nausea, or unexplained weight loss',
        description: 'Whole-body symptoms or unexplained changes are present',
      },
      'recent-trauma': {
        label: 'Recent fall, impact, or injury',
        description: 'You recently fell, hit the area, or sprained something',
      },
      'cannot-bear-weight': {
        label: 'Cannot bear weight',
        description: 'Foot, knee, or hip pain makes standing or walking difficult',
      },
      'visible-deformity': {
        label: 'Visible joint deformity',
        description: 'The joint looks deformed, angled unusually, or clearly uneven',
      },
      'warm-red-swollen': {
        label: 'Warm, red, or swollen joint',
        description: 'The joint feels hot, looks red, or swelling is increasing quickly',
      },
      'pop-instability': {
        label: 'Pop during injury with instability',
        description: 'You heard a pop during injury and the joint now feels unstable',
      },
    },
  },
  detail: {
    backToLibrary: '← Back to library',
    enlargeVideo: 'Enlarge video',
    sectionNavLabel: 'Exercise detail sections',
    sectionOverview: 'Overview',
    sectionSteps: 'Steps',
    sectionSafety: 'Safety',
    sectionAdjustments: 'Adjustments',
    sectionStart: 'Start',
    steps: 'Steps',
    benefits: 'Benefits',
    trainingSettings: 'Training settings',
    safetyCautions: 'Safety cautions',
    stopRules: 'Stop rules',
    regressions: 'Easier option',
    progressions: 'Progressions',
    startSession: 'Start training',
    catalogOnlyNotice: 'This exercise is for reference only; guided training is not available yet.',
    equipment: 'Equipment',
    defaultStopRule: 'Stop if pain rises above 3/10, numbness or tingling appears, or movement quality drops. Use an easier version next time.',
  },
  session: {
    safetyRequiredTitle: 'Safety check required',
    safetyRequiredBody: 'Please complete the safety check before starting any training.',
    goSafety: 'Go to safety check',
    blockedTitle: 'Skip training today',
    blockedBody: 'Your safety check includes red flags. Consider consulting a physician or physical therapist before home training.',
    redoSafety: 'Redo safety check',
    painBefore: 'Pain level before starting',
    painRequiredBefore: 'Choose today\'s pain level before starting. If it is 0, confirm no pain.',
    confirmNoPain: 'Confirm 0 / no pain',
    recoveryCopy: 'Pain is above 3 today. Use recovery mode: fewer reps, smaller range, slower speed, or an easier version.',
    recoverySuggestion: 'Easier option: {value}',
    useRecoveryMode: 'Use recovery mode today',
    recoveryActive: 'Recovery mode is on. Keep the movement smaller and slower today.',
    painBlockCopy: 'Pain is 6 or higher. Skip this training today and consider checking with a physician or physical therapist.',
    start: 'Start training',
    back: 'Back',
    exitSession: 'Exit session',
    exitTitle: 'Exit session',
    exitConfirmCopy: 'Are you sure you want to leave this session? Current progress is not complete.',
    continueSession: 'Continue session',
    saveAndExit: 'Save and exit',
    exitWithoutSaving: 'Exit without saving',
    exitPainAfterRequired: 'Training has started. Choose your after-training pain level before saving and exiting.',
    finishTitle: 'Log result',
    painAfter: 'Pain level after finishing',
    painRequiredAfter: 'Choose pain level after training before saving the log.',
    painAfterWarning: 'Pain rose noticeably or reached 6 or higher. Stop this exercise next time and consider professional guidance.',
    difficulty: 'How effortful did this feel today? (0–10)',
    currentDifficulty: 'Current: {value} / 10',
    difficultyLabels: {
      0: 'No effort', 1: 'Very easy', 2: 'Easy', 3: 'Light', 4: 'A little light',
      5: 'Just right', 6: 'A little hard', 7: 'Hard', 8: 'Very hard', 9: 'Extremely hard', 10: 'Max effort',
    },
    notes: 'Notes',
    notesPlaceholder: 'Example: right side felt tighter; reduced one set today',
    stopReason: 'Reason for stopping',
    saveLog: 'Save log',
    outcomePromptTitle: 'Add a function check-in?',
    outcomePromptBody: 'Your training log is saved. A quick function check-in can help you compare how daily movement feels over time.',
    outcomePromptSaved: 'This is optional. Your session is already saved, and skipping will not remove the log.',
    outcomePromptAdd: 'Add now',
    outcomePromptSkip: 'Skip',
    savedTitle: 'Session saved',
    savedLocally: 'Your session is saved in this browser on this device.',
    savedPain: 'Pain {before} → {after}',
    savedCompleted: 'Completed {completed} of {planned} sets.',
    savedStoppedEarly: 'Stopped early after {completed} of {planned} sets.',
    savedAddOutcome: 'Add a quick check-in',
    savedReturnHome: 'Finish and return home',
    savedViewRecords: 'View saved record',
    saveError: 'This session could not be saved in this browser. Please free device storage or adjust browser privacy settings, then try again.',
    activeHint: 'Move slowly, stay in control, and keep breathing. Stop right away for sharp pain, numbness, or dizziness.',
    currentCue: 'Focus now',
    instructionsAndSafety: 'Instructions and safety',
    stepsTitle: 'Full steps',
    cautionsTitle: 'Keep in mind',
    stopRulesTitle: 'Stop if',
    currentProgress: 'Current progress',
    progressA11y: 'Current set {set} of {sets}. Do {reps} reps.',
    progressText: 'Set {set} / Rep {rep}',
    targetText: 'Target {sets} sets x {reps} reps',
    nextRep: 'Next rep',
    stopSession: 'Stop training',
    earlyStopDefault: 'Felt discomfort or chose to stop early.',
  },
  restTimer: {
    title: 'Rest timer',
    seconds: '{count} sec',
    status: '{count} seconds remaining in the rest timer.',
    pause: 'Pause',
    start: 'Start',
    reset: 'Reset timer',
  },
  logs: {
    title: 'Training Logs',
    subtitle: 'Guided sessions, finished workouts, and activity records are saved on this device.',
    startTraining: 'Start today’s training',
    startTrainingHint: 'Choose an exercise in the library, then follow the safety, movement, and save flow to complete one session.',
    empty: 'No training logs yet. Finished trainings will appear here.',
    savedExerciseFallback: 'Saved exercise',
    stoppedEarly: 'Stopped early',
    pain: 'Pain {before} → {after}',
    volume: 'Completed {sets}/{plannedSets} sets, {reps}/{plannedReps} reps',
    effort: 'Effort: {value}/10',
    setDetails: 'Set details (optional)',
    setDetailsHint: 'Use this for strength work when weight or per-set reps are useful. Leave it empty for bodyweight or mobility work.',
    setNumber: 'Set {number}',
    weightKg: 'Weight (kg, optional)',
    repsPerSet: 'Reps',
    setCompleted: 'Set completed',
    addSet: 'Add set',
    removeSet: 'Remove',
    editSetDetails: 'Edit weight × reps',
    saveSetDetails: 'Save set details',
    setDetailsSaved: 'Set details saved.',
    setDetailsError: 'Set details could not be saved. Your existing log was kept.',
    setLimitReached: '20-set limit reached',
    setSummaryLabel: 'Weight and reps by set',
    noWeight: 'No weight',
    weightValue: '{value} kg',
    completedSet: 'completed',
    partialSet: 'partial',
    setSummary: 'Set {number}: {weight} × {reps} reps · {status}',
  },
  manualWorkout: {
    entry: 'Log a finished workout',
    back: 'Back to training records',
    title: 'Log a workout',
    hint: 'Record the exercises and sets you actually completed. This retrospective record does not start a guided session or assume a pain score.',
    date: 'Workout date',
    exerciseNumber: 'Exercise {number}',
    name: 'Exercise name',
    equipment: 'Equipment (optional)',
    setNumber: 'Set {number}',
    weight: 'Weight kg (optional)',
    reps: 'Reps',
    addSet: 'Add set',
    removeSet: 'Remove set',
    addExercise: 'Add exercise',
    removeExercise: 'Remove exercise',
    cyclingMinutes: 'Cycling minutes in this workout (optional)',
    cyclingHint: 'This records actual time only. It does not create a separate cycling activity or change weekly cycling advice.',
    cyclingSummary: 'Cycling {minutes} minutes',
    save: 'Save this workout',
    saveError: 'Could not save. Check the date and reps for each set; existing records were not overwritten.',
    saved: 'Workout saved. Find it in recent activity or detailed history below.',
    recordTitle: '{date} · logged workout',
    setSummary: 'Set {number} {weight} × {reps} reps',
    noWeight: 'weight not recorded',
    weightValue: '{value} kg',
    noPainData: 'Pain scores were not provided; this record does not inform rehab recommendations.',
  },
  video: {
    fallback: 'The video is temporarily unavailable. Please follow the written steps and safety reminders first.',
    search: 'Search demo video',
    title: '{title} demo video',
    defaultTitle: 'Rehab exercise demo video',
  },
  typeLabels: {
    mobility: 'Mobility',
    strength: 'Strength',
    stretch: 'Stretch',
    relaxation: 'Relaxation',
    balance: 'Balance',
    proprioception: 'Proprioception',
  },
  levelLabels: {
    beginner: 'Beginner',
    intermediate: 'Intermediate',
    advanced: 'Advanced',
  },
  equipmentLabels: {
    bodyweight: 'Bodyweight / No equipment',
    dumbbell: 'Dumbbell',
    kettlebell: 'Kettlebell',
    chair: 'Chair',
    wall: 'Wall',
    resistance_band: 'Resistance band',
    foam_roller: 'Foam roller',
  },
};
